    ...
    <ul className={`grid gap-5 ${col}`}>
      {cards.map((c) => (
        <li key={c.href + c.title}>
          <Link
            href={c.href}
            className="group flex h-full flex-col rounded-[var(--radius-card)] border border-line bg-white p-5 no-underline shadow-[var(--shadow-sm)] transition-[transform,box-shadow] duration-200 hover:-translate-y-0.5 hover:border-brand hover:shadow-[var(--shadow-float)]"
          >
